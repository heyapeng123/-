...
<body>
  ...
  ResultSet rs=db.executeQuery("select * from yuanliao where 序号="+id);
rs.next();
   %>
    <form  method="post" action="yuanliaomodify.jsp">
    <table style="text-align: center;margin: auto;font-size: 25px" >
    <tr>
    <%out.print("<td  colspan=2 align='center' height='100' width='500'>修改原料第"+id+"条记录</td>"); %>
    </tr>
    <%out.print("<input  type='hidden' name='id' value="+rs.getInt(1)+">"); %>
    <tr>
    <td width="500" height="100">原料编号：<%out.print("<input type='text' name='yuanliaobianhao' value="+rs.getString(2)+" >"); %>
    </td>
   
    <td width="500" height="100">原料名称：<%out.print("<input type='text' name='yuanliaomingcheng' value="+rs.getString(3)+">"); %>
    </td>
    </tr>
    
    <tr>
    <td width="500" height="100">市场价格：<%out.print("<input type='text' name='shichangjiage' value="+rs.getFloat(4)+" >"); %>
    </td>
