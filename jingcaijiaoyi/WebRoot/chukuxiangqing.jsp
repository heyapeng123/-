...
<html>
  ...
  <body>
   ...
   currentPage=1;
    ...
    <td width="200" height="50px">出库时间</td>
  ...
  <% 
    ...
    <td width="200" height="50px"><%if(rs.equals("")){%>未出库<% }else{%><%=rs.getDate(10) %>&nbsp;<%=rs.getTime(10) %><%} %></td>
  ...
  db.close();%>
    ...
    <center>
    <a href="chukuxiangqing.jsp?currentPage=<%=currentPage-1 %>&id=<%=id%>&nid=<%=nid %>">上一页</a>&nbsp;
    <a href="chukuxiangqing.jsp?currentPage=<%=currentPage+1 %>&id=<%=id%>&nid=<%=nid %> ">下一页</a>
    &nbsp;&nbsp;共<%=totalPage %>页
   </center>
